...
Assembler::Assembler(MM::MM& muncher, std::vector<TAC>& _instr) : muncher(muncher), instr(_instr) {}
...
void Assembler::assemble_proc(std::ofstream& os, std::size_t start, std::size_t finish) {
    ...
    // for (std::size_t i = 0; i < instr[start].get_args().size(); i++) {
    //     auto reg = stack_register("%" + std::to_string(mx + 1 + i));
    //     os << "\tmovq " << arg_registers[i] << ", " << reg << "\n";
    //     set_param_register(i, reg);
    // }
